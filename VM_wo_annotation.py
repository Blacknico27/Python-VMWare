...
def list_sub_elements(folder):
        ...
        for element in folder:
                if find_element_type(element) == "Folder":
                        ...
                        annotation = element.config.annotation
                        annotation = annotation.replace("\r","")
                        annotation = annotation.replace("\n","")
                        annotation = annotation.replace(" ","")

                        if len(annotation) == 0 and element.config.template == False:
                                #print("La machine virtuelle "+element.name+" n'a pas de commentaire !")
...
if len(host_list) != 0:
        ...
        #préparation du mail
        message = "Bonjour,\r\n\r\nCet email a pour but de vous informer de la liste des machines VMWare qui n'ont pas de commentaire.\r\n\r\n"
        message = message+"Il y a "+str(host_counter)+" machines sans commentaire.\r\n\r\n"
        for host in host_list:

                message = message+host+"\r\n"

        message = message+"\r\nMerci d'ajouter un commentaire rapidement !"
